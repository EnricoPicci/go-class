...
func (c *Customer) Score(wg *sync.WaitGroup) {
	...
	o := c.lastOrder()
	// ==>>>>>>>>> Unlock the mutex on the customer as soon as the work with the customer has completed
	fmt.Printf("Score 3 - Unlock costumer %p\n", c)
	c.mu.Unlock()
	fmt.Printf("Score 4  - Custumer %p unlocked\n", c)
	// ==>>>>>>>>> Call the setSiscount method outside the mutex protection of the customer
	o.SetDiscount(0.1)
}
...
func (o *Order) Issue(wg *sync.WaitGroup) {
	...
	o.Status = "confirmed"
	// ==>>>>>>>>> Unlock the mutex on the order as soon as the work with the order has completed
	fmt.Printf("Issue 3 - Unlock order %p\n", o)
	o.mu.Unlock()
	fmt.Printf("Issue 4  - order %p unlocked\n", o)
	// ==>>>>>>>>> Call the AddReceiveble method outside the mutex protection of the customer
